...
#' Thao: the meaning of @ and \code or \link
#'
#' @description
...
redraw_choices_from_data <- function(Rprobit_obj, seed = 1, thetas = NULL) {
  ...
  # check, if the object is right #Thao: Check if the class of Rprobit_o is Rprobit_cl
  ...
  if (Rprobit_o$mod$ordered == TRUE) {
    ...
    for (n in 1:length(Rprobit_o$data$data)) {
      ...
      # extract system
      b <- par$b
      L_Om <- par$Omega_chol #Thao?
      L_Sig <- par$Sigma_chol #Thao?
      tauk <- par$tauk #Thao?
      ...
      Xb <- X %*% b
      utils = Xb + L_Sig %*% stats::rnorm(alt) #Thao?
      # find categories
      for (j in 1:dim(X)[1]) {
        y[j] <- sum(utils[j] > tauk) + 1 #Thao?
      }
    ...
    #Thao: replace y value in Rprobit_obj$data and dataraw based on X in Rprobit_obj but b and sigma taken from build_par_from_mod and re from rnom
    cur <- 0 #Thao: n <- 1
    for (n in 1:length(Rprobit_o$data$data)) { 
      ...
      # obtain model #Thao: return different values like from a model
      par <- build_par_from_mod(thetas[choice_theta, ], Rprobit_o$mod)

      # extract system
      b <- par$b #beta differently due to random effect in the coef
      L_Om <- par$Omega_chol #Thao: Omega here is 0 as the mixing included in b as well and its var is 1 from the simu_Bui file
      ...
      y <- data_list$y
      for (j in 1:length(data_list$X)) { #Thao j <- 1
        X <- data_list$X[[j]]
        Xb = X %*% b
        utils = Xb + L_Sig %*% stats::rnorm(alt) #Thao: draw random factor from standard normal distribution
  ...
  return(Rprobit_o) # return object, although not necessary due to R6 class.#Thao?
}
